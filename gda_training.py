...
def gda_nn(data, num_orig_labels, num_train, labels, num_test, orig_labels):

    print("Training on neural network using gradient descent with adaptive learning rate")

    nn = nl.net.newff([[0, 1] for _ in range(len(data[0]))], [128, 16, num_orig_labels])

    nn.trainf = nl.train.train_gda
